...
## 1. Role
You are {persona - e.g. a senior Java engineer / a CLI-UX reviewer / an algorithms tutor}.
...
situation: what the project is (a collection of small plain-Java console programs - linked
list/stack/queue ADTs, sorting-and-searching exercises, CPU-scheduling prototypes), the
domain, and WHY this task matters. Self-contained - assume the reader has never seen this
codebase.}
...
`scheduling-prototypes/Proto8.java` (excerpt)
```
...
## 6. Constraints & Rules
- Tech stack: {plain Java (JDK 11+), no build tool, no dependencies; run via `just run <name>`}
...
- **Selective, not a dump.** Include only artifacts that bear on the task; summarize the rest.
  ...
  windows can be small). In this repo each program folder is self-contained and small -
  inlining one whole folder (e.g. a Proto + `Job.java`) is often fine; never all four.
...
Target: _"Hand off a scheduler crash to an external LLM for a fix."_

````text
# MASTER PROMPT - Guard Proto8 against an idle CPU gap

## 1. Role
You are a senior Java engineer who writes precise, minimal fixes.

## 2. Mission
Stop `Proto8` (a shortest-job-first CPU-scheduling prototype) from crashing when no job has
arrived at the moment the previous job finishes, without changing the scheduling order.

## 3. Background & Context
Small plain-Java console program (uni coursework). Jobs are read from stdin (cpu time,
arrival time, add-more answer); each clock tick the ready queue is insertion-sorted by CPU
time and the front job runs. No repo access - everything you need is below.

## 4. Current State
- Done: the program runs end-to-end when arrivals keep the CPU busy.
- Failing: an input with an idle gap (queue empty when a job completes) throws the
  exception below.
...
## 6. Constraints & Rules
- Plain Java, no new dependencies. Keep the output trace format unchanged.

## 7. Your Task
Return a corrected processing block that idles the clock forward until the next arrival when
the queue is empty, plus a one-paragraph explanation.

## 8. Output Format & Success Criteria
- Output: the fixed snippet only + the explanation.
- Done: the idle-gap input completes and prints the averages; busy inputs behave exactly as
  before.

## 9. Open Questions & Assumptions
- Assume idle ticks are not counted toward any job's waiting time.

## Reminder
Return the fixed idle-gap handling + a one-paragraph explanation. No other behavior changes.
````
...
- Ported from the marks-counter pilot for java-data-structures - same 9-section framework and
  dual-mode (agentic brief vs cold master prompt); worked example and stack references
  rewritten around this collection (Proto8 idle-gap crash, per-folder inlining guidance).
