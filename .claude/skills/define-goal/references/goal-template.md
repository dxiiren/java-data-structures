...
- {Step to bring the world up + how to check it's up + what to do if it's down. e.g.
  `pwsh ./setup.ps1` if `javac` is missing; sanity gate: `just build` then `just run` both exit 0
  with no stack trace. Reference the exact command.}
...
- {Any project house-rule that applies, e.g. "keep `sample-inputs\<name>.txt` in sync with any
  Scanner read-order change in that program".}
...
- **NOT a blocker (keep working):** "it's late", "it's complex", "needs a focused session", missing
  fixture data (-> create it / extend `sample-inputs\<name>.txt`), a stale `out\` build (-> `just clean` +
  rebuild, continue).
...
A finite, attended-ish goal - harden the known crash points in the scheduling prototypes:

```markdown
# GOAL - Harden the known crash points in Proto5-Proto8
...
Make each scheduling prototype survive an idle CPU gap (empty ready queue at job completion)
and malformed numeric input with a clear message instead of a raw exception. Success =
`just run Proto5` ... `just run Proto8` all still exit 0 on the committed
`sample-inputs\Proto*.txt`, and each crash case ends with a clear message instead of a stack
trace.

## Locked decisions (do not re-litigate)

- Keep the existing clock-trace output format byte-for-byte for well-formed input.
- No new dependencies, no build tool - plain `javac`/`java` stays.

## Environment bootstrap

- Sanity gate before starting: `just build-all` exits 0 and `just run Proto8` exits 0
  (no stack trace).
...
- Touch only `scheduling-prototypes/*.java` (+ `sample-inputs/Proto*.txt` if a new case is
  added).

## What counts as a REAL blocker

- Real: a case that cannot be guarded without restructuring the whole clock loop - cite the
  `file:line` and describe the restructure needed.
- Not a blocker: "the loop flags are messy" - slice the cases and harden them one at a time.

## Definition of Done

Per item - `DONE` only when:

- [ ] The crash case ends cleanly (shown by a `just run-interactive` transcript excerpt)
- [ ] `just run Proto<N>` on the committed sample input still exits 0 with unchanged trace
- [ ] Evidence pasted into the Run Log (exit codes + transcript lines)

Global: **G1** all four prototypes hardened. **G2** `just build-all` + the four `just run
Proto<N>` green at the end.

## Work List

| #   | Item                                        | Status |
| --- | ------------------------------------------- | ------ |
| 1   | Proto5 (`queue.get(0)` on empty queue)      | `TODO` |
| 2   | Proto6 (idle gap + arrival bookkeeping)     | `TODO` |
| 3   | Proto7 (idle gap + stats arrays)            | `TODO` |
| 4   | Proto8 (idle gap + stats arrays)            | `TODO` |
