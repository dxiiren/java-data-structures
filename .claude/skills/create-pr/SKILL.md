...
`github.com/dxiiren/java-data-structures`.
...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `fix(scheduling): guard Proto8 against an empty ready queue` or
  `docs(input): document the sample-inputs format`. Pick the type/scope from the
...
- <how it was verified: `just build` + `just run` exit codes / output excerpt, manual
  `just run-interactive` check, etc.>
```
...
title used. This repo has no CI — verification is the `just build` / `just run`
...
| `/pre-pr-review` | Optional self-review (Java CLI checklist) before opening the PR               |
