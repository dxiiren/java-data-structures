...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `harden-mark-input-parsing`), **not**
     this skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   ...
   both must exit 0 with no stack trace) to catch drift or undocumented breakage. Building a
...
- Files: `scheduling-prototypes/Proto8.java:66-155`, `sample-inputs/Proto8.txt`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just list`, `just build-all`, `just run <name>`, `just run-interactive <name>`
...
- `sample-inputs\<name>.txt` in sync with each touched program's Scanner read order:
  {yes/no as of close}
...
- Ported from the marks-counter pilot for java-data-structures - same pointer-based,
  opt-in-resume design; commands + freshness checks adapted to the collection recipes
  (`just list` / `just build-all` / `just run <name>` with per-program sample inputs).
