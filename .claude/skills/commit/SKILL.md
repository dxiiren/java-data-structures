...
2. Run **Step 0** (clear stale lock), then go straight to `git add -A` -> `git commit`
   ...
   2-second `git status` scan and exclude only those. Build output `out/` is
   git-ignored already.)
...
| Changed paths                                       | scope        |
| --------------------------------------------------- | ------------ |
| `adts/` (LinkedList, Node, Stack, Queue)            | `adts`       |
| `sorting-searching/` (ProgramApp, V1–V5, Student)   | `sorting`    |
| `scheduling-prototypes/` (Proto5–8, Job)            | `scheduling` |
| `misc/` (ForEachExample1)                           | `misc`       |
| `sample-inputs/`                                    | `input`      |
...
scope: adts, sorting, scheduling, misc, input, tooling, docs, skills
```
...
- Ported from the marks-counter pilot for java-data-structures: same fast-path/scoped
  pipeline and stale-lock preflight (no pre-commit hooks here either); scopes remapped to
  the four program folders of this collection (adts / sorting / scheduling / misc).
